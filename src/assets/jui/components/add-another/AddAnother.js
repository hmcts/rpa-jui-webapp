...
AddAnother.prototype.onAddButtonClick = function(e) {
    ...
    this.getItems().last()
        .after(item);
    item.find('input, textarea, select').first()
        .focus();
};
...
AddAnother.prototype.getNewItem = function() {
    const item = this.getItems().first()
        .clone();
...
AddAnother.prototype.updateAttributes = function(index, item) {
    item.find('[data-name]').each((i, el) => {
        el.name = $(el).attr('data-name')
            .replace(/%index%/, index);
...
AddAnother.prototype.onRemoveButtonClick = function(e) {
    $(e.currentTarget).parents('.jui-add-another__item')
        .remove();
